...
function! GitLog(args)
	" GitLog a
	if a:args == 'a' || a:args == ''
		let git_output = ' lgv --all'
	" GitLog -
	elseif a:args == '-'
		let git_output = ' lgv -- ' . s:Expand('%')
	" GitLog r
	elseif a:args == 'r'
		let git_output = 'reflog'
	" GitLog <other-args>
	else
		let git_output = 'log ' . a:args . ' -- ' . s:Expand('%')
	endif

	let git_output = s:SystemGit(git_output)
    call <SID>OpenGitBuffer(git_output)
...
command! -nargs=+ GitVim              call GitDoCommand(<q-args>)
