...
syn cluster t2tBeautifiers contains=t2tStrike,t2tUnderline,t2tItalic,t2tBold,t2tMonospace,t2tRaw,t2tTag,t2tEmph,t2tNote
...
syn match t2tUrl   '\<\(\(https\=\|ftp\|news\|telnet\|gopher\|wais\)://\([A-Za-z0-9._-]\+\(:[^ @]*\)\=@\)\=\|\(www[23]\=\.\|ftp\.\)\)[A-Za-z0-9%._/~:,=$@&-]\+\>/*\(?[A-Za-z0-9/%&=+;.,@*_-]\+\)\=\(#[A-Za-z0-9%._-]\+\)\=' contains=t2tUrlBody
...
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
"Xell: syntax
syn match t2tTag   '{{!\S\(\|.\{-}\S\)}}'
syn match t2tEmph  '((!\S\(\|.\{-}\S\)))'
syn match t2tNote  '((_\S\(\|.\{-}\S\)))'

" Conceal the link part of t2tUrl
syn match t2tUrlBody contained 'ttps\?:\/\/[^]]\+' conceal

" Highlight the first level and last blank row for lists
syn match t2tListFirstLevel '^\s[-+:]\s*'
syn match t2tListLastLine '^\s[-+:]$'

syn clear t2tHeaderArea
syn clear t2tTopArea
...
hi default link t2t_Verb       PreProc
...
hi default link t2tQuote Comment
hi default t2tTableAlign  term=reverse     cterm=reverse     gui=reverse
if &background == "light"
    hi default link t2tComment Comment
else
    hi default link t2tComment Comment
endif
hi default link t2tCommentArea Comment
"
...
hi default link t2tBlank         Underlined
...
hi default link t2tList          Constant
hi default link t2tMacro         t2t_Component
hi default link t2tTitleMark     Statement
hi default link t2tNumTitleMark  Statement
...
hi default link t2tTag   Special
hi default link t2tNote  Underlined
hi default link t2tEmph  Todo

hi default link t2tUrlBody Ignore

hi default link t2tListFirstLevel  Type
hi default link t2tListLastLine    Comment
