...
function! T2TConverter(input, out_type, config, out_path)
	...
	if a:out_path == ''
		...
		let output_file_raw = a:out_path . '/' . xelltoolkit#fname_name(input) . '.' . g:t2t_target_ext[a:out_type]
	endif
	...
	if a:config == 'normal' && a:out_type == 'html'
		let style_file = xelltoolkit#fname_escape(g:t2t_cf_path . '/css/normal.css')
		let config_file = xelltoolkit#fname_escape(g:t2t_cf_path . '/t2tcf_html.t2t')
		let config = ' --style=' . style_file . ' --config-file=' . config_file
	elseif a:config == 'note' && a:out_type == 'html'
		let style_file = xelltoolkit#fname_escape(g:t2t_cf_path . '/css/note.css')
		let config_file = xelltoolkit#fname_escape(g:t2t_cf_path . '/t2tcf_n_html.t2t')
		let config = ' --style=' . style_file . ' --config-file=' . config_file

	" XXX hack : use a:out_type for config, don't care about a:config
	else
		let config_file = xelltoolkit#fname_escape(g:t2t_cf_path . '/t2tcf_' . a:out_type . '.t2t')
