	...
	elseif (has('macunix') && (viewer =~ '^ *\(Skim\|PDFView\|TeXniscope\)\( \|$\)'))
		...
		if viewer =~ '^ *\(xdvi\|xdvik\|kdvi\|okular\)\( \|$\)'
			...
			if has("win32") || has("win64")
				let execString = 'silent! !sumatrapdfview.exe  -pdffile=' . mainfnameRoot . '.' . s:target .' -srcfile=% -line=' . line('.') . ' -editor="' . $vimruntime . '\gvim.exe -c \":RemoteOpen +\%l \%f\""'
			else
				let execStricg = 'silent! !'.viewer.' "'.mainfnameRoot.'.'.s:target.'" '.line('.').' "'.expand('%').'"'
			endif
